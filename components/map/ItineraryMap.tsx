...
export default function ItineraryMap({ activities, activeActivityId, onActivityClick, traceLiveLocation }: ItineraryMapProps) {
  ...
  const addMarkersAndRoutes = () => {
    ...
    acts.forEach((activity, index) => {
      ...
      // Use activity type to decide label (number for activities, emoji/icon for restaurants/attractions)
      const isRest = (activity as any).type === 'restaurant';
      const isAttraction = (activity as any).type === 'attraction';
      label.textContent = isRest ? "🍴" : isAttraction ? "🌟" : String(index + 1);
      label.style.cssText = `
        transform: rotate(45deg);
        color: white;
        font-size: ${isRest || isAttraction ? '14px' : '12px'};
        font-weight: 700;
        display: block;
      `;
