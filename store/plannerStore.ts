...
interface PlannerState {
  ...
  addActivity: (dayNumber: number, activity: any) => void;
    ...
    (set) => ({
      ...
      addActivity: (dayNumber, activity) =>
        set((state) => {
          ...
          const updatedDays = state.currentItinerary.days.map((day: any) => {
            ...
            return { ...day, activities: [...activities, activity] };
          });
          return { currentItinerary: { ...state.currentItinerary, days: updatedDays } };
        }),
      deleteActivity: (dayNumber, activityId) =>
        set((state) => {
          ...
          const updatedDays = state.currentItinerary.days.map((day: any) => {
            if (day.dayNumber !== dayNumber) return day;
            const activities = (day.activities || []).filter((act: any) => act.id !== activityId);
