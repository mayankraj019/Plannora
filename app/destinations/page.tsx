...
export default function DestinationsPage() {
  ...
  useEffect(() => {
    async function fetchCountries() {
      try {
        // Fetch from a stable public JSON backup since REST Countries API v3.1 is deprecated
        const res = await fetch("https://gist.githubusercontent.com/yesoreyeram/2433ce69862f452b9d0460c947ee191f/raw/");
        const data = await res.json();
        
        const tags = ["Beach", "City", "Culture", "History", "Luxury", "Nature"];
        
        const formatted = data.map((c: any, i: number) => {
          const commonName = c.name;
          const officialName = c.nativeName || c.name;
          ...
          return {
            ...
            img: `/api/images?query=${encodeURIComponent(commonName)}`,
            flag: code ? `https://flagcdn.com/${code}.svg` : ""
          };
...
<img
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 flex items-center justify-between gap-2">
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-display font-bold text-sm truncate">{city.name}</p>
                      <p className="text-[11px] font-mono truncate" style={{ color: "rgba(0,245,255,0.65)" }}>{city.country}</p>
                    </div>
                    {city.flag && (
                      <img 
                        src={city.flag} 
                        alt={`${city.name} Flag`} 
                        className="w-5 h-3.5 object-cover rounded-sm border border-white/10 shrink-0 shadow-sm"
                      />
                    )}
                  </div>
                </motion.div>
