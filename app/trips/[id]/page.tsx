      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="w-full lg:w-1/3">
            <div className="lg:sticky lg:top-8 lg:h-[calc(100vh-6rem)] lg:overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-amber/20 scrollbar-track-transparent flex flex-col gap-6 pb-6">
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-8">
                          ...
                          <div className={`p-5 rounded-2xl border transition-all ${activeId === act.id ? 'border-amber shadow-md bg-amber/5' : 'border-ivory/20 bg-ivory/10 hover:border-amber/50'}`}>
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div className="flex items-center gap-2 shrink-0 ml-2">
                                <span className="text-amber font-bold text-sm bg-amber/10 px-2 py-1 rounded-md">{act.time}</span>
                                <button
                                  onClick={() => deleteActivity(day.dayNumber, act.id)}
                                  className="p-1 rounded-md hover:bg-red-500/10 text-midnight/40 dark:text-ivory/40 hover:text-red-500 transition-colors"
            ...
            {activeTab === "guide" && (
              <div className="flex flex-col gap-6">
                ...
                {currentItinerary.famousFor && currentItinerary.famousFor.length > 0 && (
                  <div className="bg-white dark:bg-midnight/50 rounded-3xl border border-ivory/20 p-8 shadow-sm">
                    <h3 className="text-sm font-semibold uppercase tracking-wider text-amber mb-4 flex items-center gap-2">
                      🔥 What it's Famous For
                    </h3>
                    <div className="flex flex-wrap gap-2.5">
                      {currentItinerary.famousFor.map((item: string, i: number) => (
                        <span key={i} className="px-4 py-2 rounded-full text-xs font-semibold bg-amber/10 text-amber border border-amber/20 shadow-sm hover:scale-[1.02] transition-transform">
                          {item}
                        </span>
                      ))}
                    </div>
                ...
                {/* Recommended Experiences & Sights */}
                {currentItinerary.mustSeeAttractions && currentItinerary.mustSeeAttractions.length > 0 && (
                  <div className="bg-white dark:bg-midnight/50 rounded-3xl border border-ivory/20 p-8 shadow-sm">
                    <h3 className="text-sm font-semibold uppercase tracking-wider text-amber mb-6 flex items-center gap-2">
                      🌟 Recommended Experiences & Sights
                    </h3>
                    <div className="grid sm:grid-cols-2 gap-6">
                      {currentItinerary.mustSeeAttractions.map((att: any, i: number) => {
                        const isDropdownOpen = addingAttractionIdx === i;
                        return (
                          <div key={i} className="p-6 bg-ivory/5 dark:bg-midnight/30 rounded-2xl border border-ivory/20 flex flex-col gap-3 hover:border-amber/40 hover:shadow-md transition-all relative">
                            <div>
                              <h4 className="font-bold text-base text-midnight dark:text-white flex items-center gap-2 mb-2">
                                📍 {att.name}
                              </h4>
                              <p className="text-xs text-midnight/70 dark:text-ivory/70 leading-relaxed mb-3">{att.description}</p>
                              {att.whyRecommend && (
                                <p className="text-[11px] italic text-amber bg-amber/5 px-3 py-2 rounded-xl border border-amber/10 leading-relaxed">
                                  💡 <span className="font-semibold not-italic">Why recommend: </span>{att.whyRecommend}
                                </p>
                              )}
                            </div>

                            <div className="mt-auto pt-3 border-t border-ivory/10 flex justify-between items-center gap-4 relative">
                              {/* Map Pin Locator */}
                              {att.coordinates && (
                                <button
                                  onClick={() => {
                                    setActiveId(`att_${i}`);
                                    // Scroll to map
                                    const mapElement = document.querySelector(".h-\\[500px\\]");
                                    if (mapElement) {
                              ...
                              <div className="relative">
                                <button
                                  onClick={() => setAddingAttractionIdx(isDropdownOpen ? null : i)}
                                  className="text-xs font-bold bg-amber hover:bg-amber/90 text-white px-3.5 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                >
                                  Add to Itinerary
                                </button>
                                {isDropdownOpen && (
                                  <div className="absolute right-0 bottom-full mb-2 w-48 bg-white dark:bg-midnight border border-ivory/20 rounded-xl shadow-2xl p-2 z-50 flex flex-col gap-1">
                                    <p className="text-[10px] uppercase tracking-wider text-midnight/40 dark:text-ivory/40 font-bold px-2 py-1 border-b border-ivory/10">Select day</p>
                                    {Array.from({ length: currentItinerary.user_duration || 3 }).map((_, dayIdx) => {
                                      const dNum = dayIdx + 1;
                                      return (
                                        <button
                                          key={dNum}
                                          onClick={() => {
                                            const newAct = {
                                              id: `att_added_${Date.now()}_${i}`,
                                              time: "10:00 AM", // default time
                                              name: att.name,
                                              location: att.name,
                                              coordinates: att.coordinates || currentItinerary.destinationCoordinates,
                                              description: att.description,
                                              estimatedCost: "Free",
                                              category: "culture",
                                              transportToNext: { mode: "Walk", duration: "10 min" }
                                            };
                                            addActivity(dNum, newAct);
                                            setAddingAttractionIdx(null);
                                            alert(`Added "${att.name}" to Day ${dNum}!`);
                                          }}
                                          className="text-left text-xs font-semibold px-3 py-2 rounded-lg hover:bg-amber/10 hover:text-amber text-midnight/80 dark:text-ivory/80 transition-colors"
                                        >
                                          Day {dNum}
                                        </button>
                                      );
